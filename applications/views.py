...
def ajax_proc_test01(request):
	if request.method=='POST':
		...
		if post_data['req_next_token']==None:
			...
			# ↓↓↓
			# api.twitch.tv/v5が使用できなくなったのでTwitchDownloaderを参考にしてgql版のAPIから諸データを取得
			# 追加
			...
			# 長さ(秒)
			video_length=video_data['data']['video']['lengthSeconds']
			# print(video_length)
			# 1回目は空のリストを指定
			# ts_chat_dist_old=[]
			...
			DBModel.objects.update_or_create(
				...
				# 更新もしくは新規で追加したい値
				defaults={
					'md_video_title':video_data['data']['video']['title'],
				}
			)
		# 2回目以降
		else:
...
def ajax_proc_dd(request):
	if request.method=='POST':
		...
		# previewページを表示させる
			...
			json_resp=[{'videoid':videoid,'ts_chat_dist':eval(DBModel.objects.get(md_name=videoid).md_ts_chat),
									'title':DBModel.objects.get(md_name=videoid).md_video_title} for videoid in
								 request.POST.getlist('videoids')]
...
def ajax_proc_first(request):
	if request.method=='POST':
		...
		# tsとchatを辞書型リストに格納
		ts_chat_dist=[{'timestamp(JST)':comment['content_offset_seconds'],'chat':comment['message']['body']} for comment in
									row_data['comments']]
		# print(ts_chat_dist)
...
def ajax_proc_aaa_old_1(request):
	if request.method=='POST':
		...
		if post_data['req_next_token']==None:
			...
			ts_chat_url='https://api.twitch.tv/v5/videos/'+post_data['req_videoids']+'/comments?cursor='+post_data[
				'req_next_token']
			# 2回目以降はrequestsから取得
			video_length=post_data['req_video_length']
		...
		# tsとchatを辞書型リストに格納
		ts_chat_dist=[
			{'timestamp(JST)':str(video_recorded_at_jst+datetime.timedelta(seconds=int(comment['content_offset_seconds']))),
			 'chat':comment['message']['body']} for comment in ts_chat_data['comments']]
		# print(ts_chat_dist)
...
def ajax_proc_aaa_old_3(request):
	if request.method=='POST':
		...
		# tsとchatを辞書型リストに格納
		# tsは小数点以下も含まれるので切り捨て
		ts_chat_dist=ts_chat_dist_old+[
			{'ts':math.floor(int(comment['content_offset_seconds'])),'chat':comment['message']['body']} for comment in
			ts_chat_data['comments']]
		# print(ts_chat_dist)
...
def ajax_proc_aaa(request):
	if request.method=='POST':
		...
		# 経過時間をhh:mm:ss形式に変換
		ts_chat_dist=ts_chat_dist_old+[
			{'ts':str(datetime.timedelta(seconds=math.floor(int(comment['content_offset_seconds'])))),
			 'chat':comment['message']['body']} for comment in ts_chat_data['comments']]
		# print(ts_chat_dist)
...
def ajax_proc_test01_old_1(request):
	if request.method=='POST':
		...
		# tsは小数点以下も含まれることがあるので切り捨て
		# 経過時間をhh:mm:ss形式に変換
		ts_chat_dist_new=[{'ts':str(datetime.timedelta(seconds=math.floor(int(comment['content_offset_seconds'])))),
											 'chat':comment['message']['body']} for comment in ts_chat_data['comments']]
		ts_chat_dist=ts_chat_dist_old+ts_chat_dist_new
		# print(ts_chat_dist)
		# print('ts_chat_dist_new',ts_chat_dist_new)
